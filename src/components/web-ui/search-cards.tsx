    ...
    <div className="flex flex-col mt-4">
      {data?.results && data.results.length > 0 ? (
        data.results.map((item) => (
          <Link href={`/web-series/${item.id}`} key={item.id} className="group">
            ...
            >
              ...
              <div className="flex flex-col ml-4 pr-4 py-2 gap-1.5">
                <p className="text-base md:text-lg font-display font-bold leading-tight group-hover:text-primary transition-colors">{item.name}</p>
                {item.first_air_date && (
